...
data class Csaf(
    ...
    val product_tree: ProductTree? = null,
    ...
    data class Document(
        ...
        val aggregate_severity: AggregateSeverity? = null,
    ...
    data class ProductTree(
        ...
        val full_product_names: List<Product>? = null,
        /** Contains a list of product groups. */
        val product_groups: List<ProductGroup>? = null,
    ...
    data class Product(
        ...
        val product_identification_helper: ProductIdentificationHelper? = null
    ) {
    ...
    data class ProductIdentificationHelper(
        ...
        val x_generic_uris: List<XGenericUri>? = null
    ) {
        ...
        init {
            ...
            if (model_numbers != null) {
                for (cg_11 in model_numbers)
                    require(cg_11.isNotEmpty()) { "model_numbers item length < minimum 1 - ${cg_11.length}" }
            ...
            if (serial_numbers != null) {
                for (cg_13 in serial_numbers)
                    require(cg_13.isNotEmpty()) { "serial_numbers item length < minimum 1 - ${cg_13.length}" }
                require(serial_numbers.isNotEmpty()) { "serial_numbers length < minimum 1 - ${serial_numbers.size}" }
            }
            if (skus != null) {
                for (cg_14 in skus)
                    require(cg_14.isNotEmpty()) { "skus item length < minimum 1 - ${cg_14.length}" }
    ...
    data class ProductGroup(
        ...
        init {
            ...
            for (cg_32 in product_ids)
                require(cg_32.isNotEmpty()) { "product_ids item length < minimum 1 - ${cg_32.length}" }
    ...
    data class Flag(
        ...
        init {
            if (group_ids != null) {
                for (cg_46 in group_ids)
                    require(cg_46.isNotEmpty()) { "group_ids item length < minimum 1 - ${cg_46.length}" }
                require(group_ids.isNotEmpty()) { "group_ids length < minimum 1 - ${group_ids.size}" }
            }
            if (product_ids != null) {
                for (cg_47 in product_ids)
                    require(cg_47.isNotEmpty()) { "product_ids item length < minimum 1 - ${cg_47.length}" }
    ...
    data class ProductStatus(
        ...
        init {
            if (first_affected != null) {
                for (cg_51 in first_affected)
                    require(cg_51.isNotEmpty()) { "first_affected item length < minimum 1 - ${cg_51.length}" }
                require(first_affected.isNotEmpty()) { "first_affected length < minimum 1 - ${first_affected.size}" }
            }
            if (first_fixed != null) {
                for (cg_52 in first_fixed)
                    require(cg_52.isNotEmpty()) { "first_fixed item length < minimum 1 - ${cg_52.length}" }
                require(first_fixed.isNotEmpty()) { "first_fixed length < minimum 1 - ${first_fixed.size}" }
            }
            if (fixed != null) {
                for (cg_53 in fixed)
                    require(cg_53.isNotEmpty()) { "fixed item length < minimum 1 - ${cg_53.length}" }
                require(fixed.isNotEmpty()) { "fixed length < minimum 1 - ${fixed.size}" }
            }
            if (known_affected != null) {
                for (cg_54 in known_affected)
                    require(cg_54.isNotEmpty()) { "known_affected item length < minimum 1 - ${cg_54.length}" }
                require(known_affected.isNotEmpty()) { "known_affected length < minimum 1 - ${known_affected.size}" }
            }
            if (known_not_affected != null) {
                for (cg_55 in known_not_affected)
                    require(cg_55.isNotEmpty()) { "known_not_affected item length < minimum 1 - ${cg_55.length}" }
                require(known_not_affected.isNotEmpty()) { "known_not_affected length < minimum 1 - ${known_not_affected.size}" }
            }
            if (last_affected != null) {
                for (cg_56 in last_affected)
                    require(cg_56.isNotEmpty()) { "last_affected item length < minimum 1 - ${cg_56.length}" }
                require(last_affected.isNotEmpty()) { "last_affected length < minimum 1 - ${last_affected.size}" }
            }
            if (recommended != null) {
                for (cg_57 in recommended)
                    require(cg_57.isNotEmpty()) { "recommended item length < minimum 1 - ${cg_57.length}" }
                require(recommended.isNotEmpty()) { "recommended length < minimum 1 - ${recommended.size}" }
            }
            if (under_investigation != null) {
                for (cg_58 in under_investigation)
                    require(cg_58.isNotEmpty()) { "under_investigation item length < minimum 1 - ${cg_58.length}" }
    ...
    data class Remediation(
        ...
        init {
            ...
            if (entitlements != null) {
                for (cg_61 in entitlements)
                    require(cg_61.isNotEmpty()) { "entitlements item length < minimum 1 - ${cg_61.length}" }
                require(entitlements.isNotEmpty()) { "entitlements length < minimum 1 - ${entitlements.size}" }
            }
            if (group_ids != null) {
                for (cg_62 in group_ids)
                    require(cg_62.isNotEmpty()) { "group_ids item length < minimum 1 - ${cg_62.length}" }
                require(group_ids.isNotEmpty()) { "group_ids length < minimum 1 - ${group_ids.size}" }
            }
            if (product_ids != null) {
                for (cg_63 in product_ids)
                    require(cg_63.isNotEmpty()) { "product_ids item length < minimum 1 - ${cg_63.length}" }
                require(product_ids.isNotEmpty()) { "product_ids length < minimum 1 - ${product_ids.size}" }
            }
        }
    ...
    data class Score(
        ...
        init {
            for (cg_65 in products)
                require(cg_65.isNotEmpty()) { "products item length < minimum 1 - ${cg_65.length}" }
    ...
    data class CvssV2(
        ...
        val accessVector: AccessVector? = null,
        val accessComplexity: AccessComplexity? = null,
    ...
    data class Threat(
        ...
        init {
            require(details.isNotEmpty()) { "details length < minimum 1 - ${details.length}" }
            if (group_ids != null) {
                for (cg_67 in group_ids)
                    require(cg_67.isNotEmpty()) { "group_ids item length < minimum 1 - ${cg_67.length}" }
                require(group_ids.isNotEmpty()) { "group_ids length < minimum 1 - ${group_ids.size}" }
            }
            if (product_ids != null) {
                for (cg_68 in product_ids)
                    require(cg_68.isNotEmpty()) { "product_ids item length < minimum 1 - ${cg_68.length}" }
                require(product_ids.isNotEmpty()) { "product_ids length < minimum 1 - ${product_ids.size}" }
            }
        }
